...
```{r data}
# Code for question 1! 
surveys <- read_csv(here("supporting_artifacts", "Lab 2", "surveys.csv"))
dim(surveys)
```
...
There are 15 columns or variables in the data and 30,463 rows or observations
...
There are 7 chr data type variables, 7 dbl data type or number variables and one date data type.
...
# Code for question 4! 
ggplot(data = surveys, 
       mapping = aes(x = surveys$weight, y = surveys$hindfoot_length)) + 
  geom_point()
```
...
ggplot(data = surveys, 
       mapping = aes(x = surveys$weight, y = surveys$species)) +
...
##Challenge## For my challenge I colored the plot based on genus and created a custome color pallatte to specify the genus. I then annotated the plot and removed the legend so it was easy to see which species were which genus.

```{r}
# Code for question 10! 
#color deficient friendly pallatte with black 
cdPallatte_black <- c("#07437a", "#1d89ba", "#e28743", "#eab676", "#76b5c5", "#21130d", "#873e23", "#abdbe3")

ggplot(data = surveys, mapping = aes(x = surveys$weight, y = surveys$species, color = genus)) +
   geom_boxplot(outlier.shape = NA) + 
   annotate("text", y = 1, x = 290, label = "Neotoma") + 
   annotate("text", y = 2, x = 150, label = "Chaetodipus") + 
   annotate("text", y = 3, x = 150, label = "Peromyscus") + 
   annotate("text", y = 4, x = 150, label = "Perognathus") + 
   annotate("text", y = 5, x = 150, label = "Reithrodontomys") + 
   annotate("text", y = 6, x = 150, label = "Sigmodon") + 
   annotate("text", y = 7, x = 150, label = "Onychomys") + 
   annotate("text", y = 8, x = 150, label = "Peromyscus") +
   annotate("text", y = 9, x = 150, label = "Reithrodontomys") +
   annotate("text", y = 10, x = 150, label = "Dipodomys") +
   annotate("text", y = 11, x = 150, label = "Dipodomys") +
   annotate("text", y = 12, x = 150, label = "Chaetodipus") +
   annotate("text", y = 13, x = 200, label = "Dipodomys") +
   annotate("text", y = 14, x = 150, label = "Onychomys") + 
   geom_jitter(alpha = .01) + 
   labs(x = "Weight in grams", y = "Species of Animal") + 
   scale_color_manual(values = cdPallatte_black) + 
...
ggplot(data = surveys, 
       mapping = aes(y = surveys$weight, x = surveys$species)) +
   geom_boxplot(outlier.shape = NA) + 
   geom_jitter(alpha = .05, color = "blue") + 
   labs(x = "Weight in grams", y = "Species of Animal")
```
